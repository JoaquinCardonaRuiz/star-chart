...
class Plot:
    ...
    def draw_border(cls):
        ...
        for i in range(1,cls.x-1):
            cls.scr.addstr(0,i,Config.config['Chars']['DHorizontal'], curses.color_pair(color))
            cls.scr.addstr(cls.y,i,Config.config['Chars']['DHorizontal'], curses.color_pair(color))

        for j in range(1,cls.y):
            cls.scr.addstr(j,0,Config.config['Chars']['DVertical'], curses.color_pair(color))
            cls.scr.addstr(j,cls.x-1,Config.config['Chars']['DVertical'], curses.color_pair(color))

        cls.scr.addstr(0,0,Config.config['Chars']['DUpperLeft'], curses.color_pair(color))
        cls.scr.addstr(0,cls.x-1,Config.config['Chars']['DUpperRight'], curses.color_pair(color))
        cls.scr.addstr(cls.y,0,Config.config['Chars']['DLowerLeft'], curses.color_pair(color))
        ...
        try:
            cls.scr.addstr(cls.y,cls.x-1,Config.config['Chars']['DLowerRight'], curses.color_pair(color))
